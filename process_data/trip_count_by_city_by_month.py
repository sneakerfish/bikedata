...
df3 = spark.sql("select city, year(trip_start) as year, month(trip_start) as month, count(trip_start) as trip_count from bikedata group by city, year(trip_start), month(trip_start)")

df3.show(20, False)

df3.coalesce(1).write.format('com.databricks.spark.csv').save('trip_count_by_city_by_month.csv', header='true')
