...
df2 = spark.sql("select distinct(start_station_id), count(start_station_id) as trip_count, city from bikedata group by city, start_station_id")

df2.show(20, False)

df2.coalesce(1).write.format('com.databricks.spark.csv').save('station_counts.csv', header='true')
